<form role="form">
    ...
    <div class="card mb-0">
        <div class="row">
            ...
            @foreach ($fields as $key => $field)
                @if ($key === 'purchase_code'||$key==='category_id'||$key==='description')
                    {{-- Modify $field as per your requirement --}}
                    {{-- For example, if you want to change a specific value --}}
                    @php
                        // Modify the value of a specific field attribute
                        $field['type'] = 'hidden';
                        $field['hint'] = '';

                    @endphp

                    {{-- Now include the modified field --}}
                    @include('admin.panel.fields.' . $field['type'], ['field' => $field])

                    {{-- If the key is not 'purchase_code', include the field without modification --}}
                @else
                    @include('admin.panel.fields.' . $field['type'], ['field' => $field])
                @endif
            @endforeach
