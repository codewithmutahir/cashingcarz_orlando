    ...
    <div class="mb-3">
        ...
        <div class="input-group">
            <input type="text"
                   ...
                   value="{{ \App\Models\User::find(auth()->id())->referral_code }}"
            >

            <datalist id="referralCodesList">
                @foreach(\App\Models\User::select('referral_code')->whereNotNull('referral_code')->distinct()->limit(50)->get() as $user)
                    <option value="{{ $user->referral_code }}">
                @endforeach
            </datalist>

            <button type="button" class="btn btn-outline-secondary" onclick="generateReferralCode()">Generate</button>
        </div>
    </div>

    <p id="referral-status" class="text-success small"></p>
</div>

@if (auth()->check() && auth()->user()->referral_code)
    <p>Your Referral Code: <strong id="referral-code-display">{{ auth()->user()->referral_code }}</strong></p>
@endif
...
function saveReferralCode(code) {
    fetch("{{ url('/account/referral-code') }}", {
        method: 'PUT',
        headers: {
    ...
    .then(data => {
        if (data.success || data.message) {
            document.getElementById('referral-status').innerText = 'Referral code saved successfully!';
            document.querySelector('input#referral_code').value = code;
            ...
            document.getElementById('referral-status').innerText = 'Failed to save referral code.';
        }
    })
    .catch(error => {
        console.error(error);
        document.getElementById('referral-status').innerText = 'Error saving referral code.';
    });
}
	</script>
@endsection
