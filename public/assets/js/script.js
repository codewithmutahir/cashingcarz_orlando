...
$(document).ready(function () {
	...
	$('#locSearch').on('mouseenter', function () {
	...
	function fadeOnScroll(target) {
		...
		$(window).on('scroll', function () {
			...
			window.requestAnimationFrame(function () {
				...
				if (scrollPercent >= 0) {
					$target.css('background-color', 'rgba(0,0,0,' + (1.1 - scrollPercent) + ')');
				}
				ticking = false;
			});
		});
	...
	*/
	...
	/* NiceScroll: disable MutationObservers (they fight DevTools DOM updates and can freeze the tab) */
	...
	function applyCatCollapseLayout() {
		if ($(window).width() < 767) {
	...
	var catCollapseResizeTimer = null;
	$(window).on('resize', function () {
		clearTimeout(catCollapseResizeTimer);
		catCollapseResizeTimer = setTimeout(applyCatCollapseLayout, 150);
	});
	applyCatCollapseLayout();
