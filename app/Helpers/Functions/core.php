...
{
	// Same request often calls this twice (e.g. controller + layout); avoid duplicate DB work.
	static $memo = [];
	$languageCode = config('lang.code', config('app.locale'));
	$memoKey = ($page ?? '') . '|' . $languageCode;
	...
	if (!empty($model)) {
		...
		$memo[$memoKey] = array_values($metaTag);
	...
	$memo[$memoKey] = is_array($metaTag) ? array_values($metaTag) : [];
