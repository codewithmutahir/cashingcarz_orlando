...
class EditController extends AccountBaseController
{
    	...
    	public function updateDetails(UserRequest $request)
    {
        ...
        logger('Web Controller - updateDetails called');
        logger('Request data:', $request->all());
        logger('Referral code from request:', $request->input('referral_code'));
    
        // Call API endpoint
        $endpoint = '/users/' . auth()->user()->getAuthIdentifier();
    
        // ✅ Add referral_code explicitly in data
        $data = $request->all();
         // Make sure referral_code is included
         if ($request->filled('referral_code')) {
        $data['referral_code'] = $request->input('referral_code');
        logger('Adding referral_code to API data:', $request->input('referral_code'));
    }

        logger('Data being sent to API:', $data);

        // 🔄 Make the API call
        $data = makeApiRequest('put', $endpoint, $data);
        ...
        $user = data_get($data, 'result');
               ...
               // Refresh session user to reflect latest referral_code
       if ($user && isset($user['referral_code'])) {
    \App\Models\User::where('id', auth()->id())
        ->update(['referral_code' => $user['referral_code']]);

    auth()->setUser(\App\Models\User::find(auth()->id())); // Refresh session
}
