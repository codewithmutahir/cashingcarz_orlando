...
class HomeSectionController extends BaseController
{
	...
	public function index(): \Illuminate\Http\JsonResponse
	{
		...
		if ($sections->count() > 0) {
			...
			$payloadCacheId = HomeSection::indexPayloadCacheKey($countryCode, app()->getLocale());
			
			try {
				$homeSections = cache()->remember($payloadCacheId, $this->cacheExpiration, function () use ($sections) {
					$built = [];
					...
					foreach ($sections as $method => $section) {
						...
						$built[$method]['method'] = $method;
						$built[$method]['data'] = $this->{$method}($section->value);
						$built[$method]['view'] = $section->view;
						if (method_exists($this, $settingMethod)) {
							$built[$method][$optionName] = $this->{$settingMethod}($section->value);
						} else {
							$built[$method][$optionName] = $section->value;
						}
						$built[$method]['lft'] = $section->lft;
					}
