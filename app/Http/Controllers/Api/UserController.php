...
// {
//     logger('PUT request data', $request->all());

//     // Explicitly add referral_code before passing
//     $data = $request->all();
//     $data['referral_code'] = $request->input('referral_code');

//     // Call API endpoint with referral_code included
//     $endpoint = '/users/' . $id;
//     $response = makeApiRequest('put', $endpoint, $data);

//     $message = !empty(data_get($response, 'message')) ? data_get($response, 'message') : 'Unknown Error.';

//     if (!data_get($response, 'isSuccessful')) {
...
public function update($id, UserRequest $request): \Illuminate\Http\JsonResponse
{
    ...
    // ✍️ Fill other fields (excluding photo)
    $user->fill($request->except(['photo']));

    // ✅ Explicitly set referral_code if provided
