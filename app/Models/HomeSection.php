...
class HomeSection extends BaseModel
{
	...
	*/
	...
	/**
	 * Cache key for the fully built /api/homeSections JSON payload (all section data for one locale).
	 */
	public static function indexPayloadCacheKey(string $countryCode, string $locale): string
	{
		return $countryCode . '.homeSections.payload.' . $locale;
	}
	
	/**
	 * Invalidate cached home section payloads when listings change for a country.
	 */
	public static function forgetIndexPayloadCacheForCountry(?string $countryCode): void
	{
		...
		try {
			...
			foreach ($languages as $language) {
				cache()->forget(static::indexPayloadCacheKey($countryCode, $language->code));
			}
		} catch (\Throwable $e) {
		}
	}
