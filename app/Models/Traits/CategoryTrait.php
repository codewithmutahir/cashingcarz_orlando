...
trait CategoryTrait
{
	...
	public static function countPostsPerCategory($cityId = null): array
	{
		...
		$suffix = ! empty($cityId) ? '.city.' . $cityId : '';
		// Must match keys cleared in App\Observers\PostObserver::clearCache()
		$cacheKey = $countryCode . '.count.posts.per.cat.' . $lang . $suffix;

		$ttl = (int) config('settings.optimization.cache_expiration', 3600);

		return Cache::remember($cacheKey, $ttl, static function () use ($cityId) {
			$whereCity = '';
			if (! empty($cityId)) {
				$whereCity = ' AND tPost.city_id = ' . (int) $cityId;
			}
